...
async function db(sql, args = []) {
  ...
  return (result?.rows || []).map((row) =>
    Object.fromEntries(row.map((cell, i) => [columns[i], decodeValue(cell)])),
  );
}
...
async function seed() {
  for (const [index, module] of modules.entries()) {
    await db(
      `INSERT OR REPLACE INTO learning_modules (id,title,description,icon,content,min_level,sort_order,is_published,updated_at) VALUES (?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP)`,
      [
        module.id,
        module.title,
        module.description,
    ...
    for (const [gIndex, item] of module.glossary.entries()) {
      await db(
        `INSERT OR REPLACE INTO module_glossary_items (id,module_id,term,definition,sort_order) VALUES (?,?,?,?,?)`,
        [
          `${module.id}-g-${gIndex}`,
          module.id,
          item.term,
    ...
    for (const [qIndex, question] of module.quiz.entries()) {
      await db(
        `INSERT OR REPLACE INTO module_quiz_questions (id,module_id,question,options_json,correct_index,explanation,image_url,sort_order) VALUES (?,?,?,?,?,?,?,?)`,
        [
          question.id,
          module.id,
          question.question,
...
async function getModules(moduleId) {
  ...
  const rows = await db(
    `SELECT m.*, g.id glossary_id, g.term, g.definition, q.id quiz_id, q.question, q.options_json, q.correct_index, q.explanation, q.image_url FROM learning_modules m LEFT JOIN module_glossary_items g ON g.module_id=m.id LEFT JOIN module_quiz_questions q ON q.module_id=m.id ${where} ORDER BY m.sort_order, g.sort_order, q.sort_order`,
    moduleId ? [moduleId] : [],
  );
  const map = new Map();
  ...
  .createServer(async (req, res) => {
    try {
      ...
      if (url.pathname === "/modules" && req.method === "GET")
        return json(res, 200, await getModules());
